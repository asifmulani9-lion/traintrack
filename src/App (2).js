    ...
    <div className="login-wrap"><style>{CSS}</style>
      <div className="login-card">
        ...
        <div className="login-form">
          ...
          <p className="login-hint">Admin: admin@traintrack.com / Admin@123</p>
        </div>
      </div>
    </div>
  );
...
function ReportsPanel({db}){
  ...
  return(
    <div className="section">
      ...
      <div className="filter-tabs">
        {[["attendance","📅 Attendance"],["activity","🏃 Activity"],["feedback","💬 Feedback"]].map(([id,lbl])=>(
...
function TraineesPanel({user,db,patch}){
  ...
  const masters=db.masters||{};
  const [adding,setAdding]=useState(false);
  const [leftModal,setLeftModal]=useState(null); const [leftDate,setLeftDate]=useState(today());
  const [pwModal,setPwModal]=useState(null); const [newPw,setNewPw]=useState("");
  ...
  const markLeft=()=>{patch("trainees",trainees.map(t=>t.id===leftModal?{...t,leftDate,active:false}:t));setLeftModal(null);};
    ...
    <div className="section">
      ...
      {mine.map(t=>(
        <Card key={t.id}>
          ...
          {!t.leftDate&&(
            <div className="action-row">
              <Btn small variant="secondary" onClick={()=>{setPwModal(t.id);setNewPw("");}}>Reset PW</Btn>
              <Btn small variant="danger" onClick={()=>{setLeftModal(t.id);setLeftDate(today());}}>Mark Left</Btn>
            </div>
          )}
          {pwModal===t.id&&(
            <div className="inline-add" style={{marginTop:8}}>
      ...
      {leftModal&&(
        <Modal title="Mark Trainee as Left" onClose={()=>setLeftModal(null)}>
          <Input label="Last Working Date" type="date" value={leftDate} onChange={e=>setLeftDate(e.target.value)}/>
          <Btn onClick={markLeft} variant="danger" fullWidth>Confirm</Btn>
        </Modal>
      )}
    </div>
  );
...
function TrainerReports({user,db}){
  ...
  const t=my.find(x=>x.id===sel);
  const att=(db.attendance||[]).filter(a=>a.traineeId===sel);
  const fb=(db.feedbacks||[]).filter(f=>f.traineeId===sel);
  const avg=fb.length?(fb.reduce((s,f)=>s+(f.sessionRate||0),0)/fb.length).toFixed(1):"—";
  return(
    <div className="section">
      <h2 className="section-title">Trainee Reports</h2>
      <Sel label="Select Trainee" options={my.map(x=>({value:x.id,label:x.name+(x.leftDate?" (Left)":x.active===false?" (Inactive)":"")}))} value={sel} onChange={e=>setSel(e.target.value)}/>
      {t&&(
        <>
          <div className="stat-grid">
            <div className="stat-card"><div className="stat-num accent">{att.filter(a=>a.status==="Present").length}/{att.length}</div><div className="stat-lbl">Attendance</div></div>
            <div className="stat-card"><div className="stat-num">⭐ {avg}</div><div className="stat-lbl">Avg Rating</div></div>
            <div className="stat-card"><div className="stat-num">{fb.length}</div><div className="stat-lbl">Feedbacks</div></div>
            <div className="stat-card"><div className="stat-num">{(db.activities||[]).filter(a=>a.traineeId===sel).length}</div><div className="stat-lbl">Activities</div></div>
          </div>
          <h3 className="sub-title">Recent Feedbacks</h3>
